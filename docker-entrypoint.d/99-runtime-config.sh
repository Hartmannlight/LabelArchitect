...
if [ -z "${APP_FLEET_CONSOLE_BASE:-}" ] && [ -n "${VITE_FLEET_CONSOLE_BASE:-}" ]; then
  APP_FLEET_CONSOLE_BASE="$VITE_FLEET_CONSOLE_BASE"
fi
...
export APP_BACKEND_API_BASE APP_RENDER_API_BASE APP_OPERATOR_APP_BASE APP_FLEET_CONSOLE_BASE APP_API_UPSTREAM APP_LABEL_SIZE_PRESETS

if [ -f "$TEMPLATE_PATH" ]; then
  envsubst '${APP_BACKEND_API_BASE} ${APP_RENDER_API_BASE} ${APP_OPERATOR_APP_BASE} ${APP_FLEET_CONSOLE_BASE} ${APP_LABEL_SIZE_PRESETS}' \
