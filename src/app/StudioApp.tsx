...
function QuickPrint({ onNotice }: { onNotice: (notice: Notice) => void }) {
  ...
  const [values, setValues] = useState<Record<string, string>>({}); const [previewUrl, setPreviewUrl] = useState<string | null>(null); const [busy, setBusy] = useState<'preview' | 'print' | null>(null)
  ...
  return <main className='page-shell quick-print-page'><header className='page-heading compact'><div><span className='eyebrow'>Quick print</span><h1>Choose, fill, print</h1><p>This view is designed for phones, scanners and quick repeat jobs.</p></div></header><div className='quick-print-layout'><section className='form-panel'>
    ...
    <div className='quick-actions'><button type='button' onClick={() => void preview()} disabled={!template || busy !== null}>{busy === 'preview' ? 'Rendering…' : 'Preview'}</button><button className='primary-action' type='button' onClick={() => void print()} disabled={!template || !printerId || busy !== null}>{busy === 'print' ? 'Sending…' : 'Print label'}</button></div>
...
function Designer() {
  ...
  const tools = [['select', 'Select'], ['split_v', 'Split ↔'], ['split_h', 'Split ↕'], ['place_text', 'Text'], ['place_qr', 'QR'], ['place_dm', 'DataMatrix'], ['place_image', 'Image'], ['place_line', 'Line']] as const
  useEffect(() => { const handler = (event: KeyboardEvent) => { if (!(event.ctrlKey || event.metaKey)) return; if (event.key.toLowerCase() === 'z') { event.preventDefault(); if (event.shiftKey) redo(); else undo() } if (event.key.toLowerCase() === 'y') { event.preventDefault(); redo() } if (event.key.toLowerCase() === 's') { event.preventDefault(); setStoreOpen(true) } }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler) }, [redo, undo])
  return <main className='designer-page'><div className='mobile-designer-warning'><strong>Designer is made for desktop.</strong><span>Use Quick print on this device to fill and print existing templates.</span><button type='button' className='primary-action' onClick={() => navigate('print')}>Open Quick print</button></div>
    ...
    <div className='designer-toolbar' aria-label='Designer tools'>{tools.map(([key, label]) => <button type='button' key={key} className={tool === key ? 'active' : ''} onClick={() => setTool(key)}>{label}</button>)}<span className='toolbar-spacer' /><button type='button' onClick={undo}>Undo</button><button type='button' onClick={redo}>Redo</button></div>
    <div className='designer-workspace'><aside><TreePanel /></aside><section className='canvas-stack'><div className='canvas-surface'><CanvasEditor /></div><div className='render-surface'><LabelPreviewPanel /></div></section><aside className='properties-surface'><PropertiesPanel /></aside></div><footer className='designer-status'><ValidationPanel issues={issues} /></footer>{storeOpen && <TemplateStoreDialog onClose={() => setStoreOpen(false)} />}{jsonMode && <JsonDialog mode={jsonMode} onClose={() => setJsonMode(null)} />}</main>
}
